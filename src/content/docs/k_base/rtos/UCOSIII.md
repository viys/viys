...
- `OS_TaskRdy()`函数
  ...
  ![时基列表删除TCB](assets/时基列表删除TCB.png)
...
- 在运行到`OSInit()`系统初始化函数时,系统就会将消息池进行初始化,其中`OS_MsgPoolInit()`(定义在os_msg.c中)就是消息池初始化函数.
  ...
  - OSMsgPool是个全局变量,用来管理内存池的存取操作,它包含以下四个元素.
    ...
    ![OSMsgPool](assets/OSMsgPool.png)
...
![消息队列的运作过程](assets/消息队列的运作过程.png)
...
- μC/OS的消息队列由多个元素组成,在信号量被创建时,需要由我们自己定义消息队列(也可以称之为消息队列句柄),因为它是用于保存消息队列的一些信息的,其数据结构OS_Q除了队列必须的一些基本信息外,还有PendList链表与MsgQ,为的是方便系统来管理消息队列.![消息队列的结构](assets/消息队列的结构.png)
...
![消息队列a](assets/消息队列a.png)

![消息队列b](assets/消息队列b.png)
...
1. 声明消息队列,并定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![消息队列1_1](assets/消息队列1_1.png)

   ![消息队列1_3](assets/消息队列1_3.png)

   ![消息队列1_2](assets/消息队列1_2.png)

2. 定义任务函数

   ![消息队列2_1](assets/消息队列2_1.png)

3. 任务启动函数编写

   ![消息队列2_2](assets/消息队列3.png)

4. 结果现象

   ![消息队列实验现象](assets/消息队列实验现象.png)
...
- 在二值信号量无效的时候,假如此时有任务获取该信号量的话,那么任务将进入阻塞状态.
  ...
  ![信号量无效时候获取](assets/信号量无效时候获取.png)

- 假如某个时间中断/任务释放了信号量,其过程具体见图中断/任务释放信号量,那么,由于获取无效信号量而进入阻塞态的任务将获得信号量并且恢复为就绪态.

  ![二值信号量运作机制a](assets/二值信号量运作机制a.png)

  ![二值信号量运作机制b](assets/二值信号量运作机制b.png)
...
- 计数信号量可以用于资源管理,允许多个任务获取信号量访问共享资源,但会限制任务的最大数目.访问的任务数达到可支持的最大数目时,会阻塞其他试图获取该信号量的任务,直到有任务释放了信号量.这就是计数型信号量的运作机制,虽然计数信号量允许多个任务访问同一个资源,但是也有限定,比如某个资源限定只能有3个任务访问,那么第4个任务访问的时候,会因为获取不到信号量而进入阻塞,等到有任务(比如任务1)释放掉该资源的时候,第4个任务才能获取到信号量从而进行资源的访问.
  ...
  ![计数信号量运作示机制](assets/计数信号量运作示机制.png)
...
1. 声明信号量,并定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![信号量实验1_1](assets/信号量实验1_1.png)

   ![信号量实验1_2](assets/信号量实验1_2.png)

   ![信号量实验1_3](assets/信号量实验1_3.png)

2. 定义任务函数

   ![信号量实验2](assets/信号量实验2.png)

3. 任务启动函数编写

   ![信号量试验3](assets/信号量试验3.png)
...
- μC/OS的互斥量由多个元素组成,在互斥量被创建时,需要由我们自己定义互斥量(也可以称之为互斥量句柄),因为它是用于保存互斥量的一些信息的,其数据结构`OS_MUTEX`除了互斥量必须的一些基本信息外,还有指向任务控制块的指针`OwnerTCBPtr`,任务优先级变量`OwnerOriginalPrio`,`PendList` 链表与 `OwnerNestingCtr`变量等,为的是方便系统来管理互斥量.
  ...
  ![互斥量的控制块数据结构](assets/互斥量的控制块数据结构.png)
...
1. 声明信号量,并定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![优先级翻转1_1](assets/优先级翻转1_1.png)

   ![优先级翻转1_2](assets/优先级翻转1_2.png)

   ![优先级翻转1_3](assets/优先级翻转1_3.png)

2. 定义任务函数

   ![优先级翻转2](assets/优先级翻转2.png)

3. 任务启动函数编写

   ![优先级翻转3](assets/优先级翻转3.png)

4. 结果现象

   ![优先级翻转4](assets/优先级翻转4.png)
...
1. 声明互斥信号量,并定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![互斥量实验1_1](assets/互斥量实验1_1.png)

   ![互斥量实验1_2](assets/互斥量实验1_2.png)

   ![互斥量实验1_3](assets/互斥量实验1_3.png)

2. 定义任务函数

   ![互斥量实验2](assets/互斥量实验2.png)

3. 任务启动函数编写

   ![互斥量实验3](assets/互斥量实验3.png)

4. 结果现象

   ![互斥量实验4](assets/互斥量实验4.png)
...
- 事件不与任务相关联,事件相互独立,一个32位的变量就是事件的集合,用于标识该任务发生的事件类型,其中每一位表示一种事件类型(0表示该事件类型未发生,1表示该事件类型已经发生),一共32种事件类型.
  ...
  ![事件集合Flags](assets/事件集合Flags.png)

- 事件唤醒机制,当任务因为等待某个或者多个事件发生而进入阻塞态,当事件发生的时候会被唤醒.

  ![事件唤醒任务](assets/事件唤醒任务.png)
...
- 事件控制块数据结构
  ...
  ![事件控制块数据结构](assets/事件控制块数据结构.png)
...
1. 声明事件标志组,并定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![事件实验1_1](assets/事件实验1_1.png)

   ![事件实验1_2](assets/事件实验1_2.png)

2. 定义任务函数

   ![事件实验2](assets/事件实验2.png)

3. 任务启动函数编写

   ![事件实验3](assets/事件实验3.png)

4. 结果现象

   ![事件实验4](assets/事件实验4.png)
...
- μC/OS 提供的软件定时器支持单次模式和周期模式,单次模式和周期模式的定时时间到之后都
  ...
  ![软件定时器的单次模式与周期模式](assets/软件定时器的单次模式与周期模式.png)
  ...
  - 周期模式:这个定时器会按照设置的定时时间循环执行回调函数,直到用户将定时器删除.
    ...
    ![两种周期模式运行示意图](assets/两种周期模式运行示意图.png)
...
1. 定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![软件定时器1_1](assets/软件定时器1_1.png)

   ![软件定时器1_2](assets/软件定时器1_2.png)

2. 定义任务函数

   ![软件定时器2](assets/软件定时器2.png)

3. 任务启动函数编写

   ![软件定时器3](assets/软件定时器3.png)

4. 结果现象

   ![软件定时器4](assets/软件定时器4.png)
...
1. 定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![任务信号量代替二值信号量实验1_1](assets/任务信号量代替二值信号量实验1_1.png)

   ![任务信号量代替二值信号量实验1_2](assets/任务信号量代替二值信号量实验1_2.png)

2. 定义任务函数

   ![任务信号量代替二值信号量实验2](assets/任务信号量代替二值信号量实验2.png)

3. 任务启动函数编写

   ![任务信号量代替二值信号量实验3](assets/任务信号量代替二值信号量实验3.png)

4. 结果现象

   ![任务信号量代替二值信号量实验4](assets/任务信号量代替二值信号量实验4.png)
...
1. 定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![任务信号量代替计数信号量实验1_1](assets/任务信号量代替计数信号量实验1_1.png)

   ![任务信号量代替计数信号量实验1_2](assets/任务信号量代替计数信号量实验1_2.png)

2. 定义任务函数

   ![任务信号量代替计数信号量实验2](assets/任务信号量代替计数信号量实验2.png)

3. 任务启动函数编写

   ![任务信号量代替计数信号量实验3](assets/任务信号量代替计数信号量实验3.png)

4. 结果现象

   ![任务信号量代替计数信号量实验4](assets/任务信号量代替计数信号量实验4.png)
...
1. 定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![任务消息队列实验1_1](assets/任务消息队列实验1_1.png)

   ![任务消息队列实验1_2](assets/任务消息队列实验1_2.png)

2. 定义任务函数

   ![任务消息队列实验2](assets/任务消息队列实验2.png)

3. 任务启动函数编写

   ![任务消息队列实验3](assets/任务消息队列实验3.png)

4. 结果现象

   ![任务消息队列实验4](assets/任务消息队列实验4.png)
...
1. 声明内存管理对象和内存分区,定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.
   ...
   ![内存管理1_1](assets/内存管理1_1.png)

   ![内存管理1_2](assets/内存管理1_2.png)

2. 定义任务函数

   ![内存管理2](assets/内存管理2.png)

3. 任务启动函数编写

   ![内存管理3](assets/内存管理3.png)

4. 结果现象

   ![内存管理4](assets/内存管理4.png)
...
- 中断发生的环境有两种情况:在任务的上下文中,在中断服务函数处理上下文中.
  ...
  - 在任务的上下文中![中断发生的环境1](assets/中断发生的环境1.png)
...
1. 开启延时中断
   ...
   ![中断实验1](assets/中断实验1.png)

2. 定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.

   ![中断实验2_1](assets/中断实验2_1.png)

   ![中断实验2_2](assets/中断实验2_2.png)

3. 定义任务函数

   ![中断实验3](assets/中断实验3.png)

4. 中断服务函数编写

   ![中断实验4](assets/中断实验4.png)

5. 任务启动函数编写

   ![中断实验5](assets/中断实验5.png)

6. 结果现象

   ![中断实验6](assets/中断实验6.png)
...
1. 启用宏定义`OS_CFG_STAT_TASK_EN`.
   ...
   ![统计任务实验1](assets/统计任务实验1.png)

2. 声明内存管理对象和内存分区,定义任务空间栈的大小以及任务栈数组,任务控制块和优先级.

   ![统计任务实验1_1](assets/统计任务实验2_1.png)

   ![统计任务实验1_2](assets/统计任务实验2_2.png)

3. 定义任务函数

   ![统计任务实验3](assets/统计任务实验3.png)

4. 任务启动函数编写

   ![统计任务实验4](assets/统计任务实验4.png)

5. 结果现象

   ![统计任务实验5](assets/统计任务实验5.png)
